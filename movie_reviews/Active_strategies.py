...
def active_selection(vect, strategy, sample_size, train_col, target_col, X_train, y_train, model, unlabeled_data):
    ...
    if strategy == 'random':
        ...
        X_expanded = pd.DataFrame(data=np.concatenate([X_train.values, df_temp[train_col].values.reshape((sample_size, 1))], axis=0), columns = [train_col]) 
